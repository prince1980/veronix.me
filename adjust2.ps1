...
$audioBitrate = 192000
$files = Get-ChildItem -File | Where-Object { $_.Extension -eq '.mp4' -and $_.Name -in @('6.mp4','8.mp4','4.mp4') }
foreach ($f in $files) {
  ...
  Write-Host "Retune $($f.Name) factor=$([math]::Round($factor,3)) vb=$videoBitrate"
